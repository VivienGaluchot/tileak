...
const p2p = function () {
    ...
    class PingHandler extends ChannelHandler {
        constructor() {
            ...
            this.onStateChange = () => {
                if (this.isConnected) {
                    console.debug(`PingHandler | connected`);
                } else {
                    console.debug(`PingHandler | disconnected`);
                }
            };
            this.onPingChange = () => {
                console.debug(`PingHandler | ping received in ${this.pingDelay ?? "-"} ms`);
            };
        ...
        onopen(connection, chan, evt) {
            const sendPing = () => {
                if (this.pingValue != null) {
                    console.debug("PingHandler | ping timeout");
                    this.pingDelay = null;
                ...
                try {
                    ...
                    console.warn("PingHandler | ping could not be sent", error);
                }
            ...
            let handler = new FrameHandler()
                .on("ping", data => {
                    ...
                    if (data.src == connection.localEndpoint.id) {
                        ...
                        // echo ping from remote peer
                        try {
                            chan.send(evt.data);
                        } catch (error) {
                            console.warn("PingHandler | ping could not be echoed", error);
                        }
                    }
                });
    ...
    class Hub extends ChannelHandler {
        constructor(localEndpoint) {
            ...
            this.onChange = hub => {
                console.debug("Hub | hub update, known hosts are");
                for (let con of this.connections()) {
                    console.debug(`Hub | - ${con.remoteEndpoint.id}`);
                }
            };
        ...
        addConnection(connection, chan) {
            ...
            let peerAnnouncedFrame = new Frame("known-peers", { ids: [connection.remoteEndpoint.id] });
            console.debug(`Hub | peer announced to the HUB ${connection.remoteEndpoint.id}`, peerAnnouncedFrame)

            // exchange known peers
            // they will then be in change to initiate the connections between then through a common peer if needed
            for (let peerChan of this.channels()) {
                if (peerChan == chan) {
                    ...
                    console.debug(`Hub | known-peers sent to ${connection.remoteEndpoint.id}`, frameData);
                    peerChan.send(frameData.serialize());
                } else {
                    // share the new peer id with the rest of the hub
                    peerChan.send(peerAnnouncedFrame.serialize());
                }
            }
        }
        ...
        * peersId() {
        ...
        sendRouted(chan, dst, payload) {
            ...
            chan.send(frameData.serialize());
            console.debug("Hub | send routed data", frameData);
        }
        ...
        offerRoutedConnection(chan, targetId) {
            ...
            connection.onStateChange = () => {
                if (connection.isConnected) {
                    console.debug(`Hub | auto connected to peer ${connection.remoteEndpoint.id}`);
            ...
            connection.createOffer()
                .then((offer) => {
                    console.debug("Hub | createOffer ok");
                ...
                .catch(reason => {
                    console.error("Hub | createOffer error", reason);
                    this.pendingConnections.delete(targetId);
                });
        }
        ...
        answerRoutedConnection(chan, targetId, offer) {
            ...
            connection.consumeOfferAndGetAnswer(offer)
                .then(answer => {
                    console.debug("Hub | consumeOfferAndGetAnswer ok");
                ...
                .catch(reason => {
                    console.error("Hub | consumeOfferAndGetAnswer error", reason);
                    this.pendingConnections.delete(targetId);
                });
        }
        ...
        completeRoutedConnection(targetId, answer) {
            ...
            connection.consumeAnswer(answer)
                .then(() => {
                    console.debug("Hub | consumeAnswer ok");
                })
                .catch(reason => {
                    console.error("Hub | consumeAnswer error", reason);
                    this.pendingConnections.delete(targetId);
                });
        }
        ...
        handleMessage(chan, evt, frameData) {
            let handler = new FrameHandler()
                .on("routed", data => {
                    console.debug("Hub | onmessage routed", data);
                    ...
                    if (data.dst == this.localEndpoint.id) {
                        ...
                        if (dstPeer == undefined) {
                            console.warn(`Hub | can't route message, dst unknown`, data);
                        } else {
                ...
                .on("known-peers", data => {
                    console.debug("Hub | onmessage known-peers", data);

                    // try to connect with unknown peers
                    for (let id of data.ids) {
                        if (this.localEndpoint.id == id) {
                            console.debug(`Hub | don't require connection to ${id}, local id`);
                        } else if (this.peerMap.has(id)) {
                            console.debug(`Hub | don't require connection to ${id}, already known`);
                        } else if (this.pendingConnections.has(id)) {
                            console.debug(`Hub | don't require connection to ${id}, pending connection`);
                        } else {
                            if (id < this.localEndpoint.id) {
                                console.debug(`Hub | require connection to ${id}, initiate it`);
                                this.offerRoutedConnection(chan, id);
                            } else {
                                console.debug(`Hub | require connection to ${id}, waiting for it`);
                            }
                        }
                    }
                })
                .on("connection-offered", data => {
                    console.debug("Hub | onmessage connection-offered", data);
                ...
                .on("connection-answered", data => {
                    console.debug("Hub | onmessage connection-answered", data);
